...
class Viewer:
    def __init__(self, cfg, figsize):
        self.cfg = cfg
        self.xlim = cfg.obj.length * (cfg.obj.nx - 1) / 2 + cfg.viewer.space
        self.ylim = cfg.obj.length * (cfg.obj.ny - 1) + cfg.viewer.space
    ...
    def show(self, obj, step: int):
        ...
        self.fig.suptitle(
            f"Step #{self.step},  $k = {self.cfg.obj.k}$ [N/m],  $c = {self.cfg.obj.c}$ [Nm/s],  $dt = {self.cfg.sim.dt}$ [s]"
        )

        self.imgs.append(fig_to_img(self.fig))
        plt.pause(self.cfg.sim.dt)
    ...
    def draw_ax(self, ax, title):
        ...
        # Plot external force
        top_left_node = self.obj.find_top_left_node()
        top_right_node = self.obj.find_top_right_node()
        _draw_arrow(top_left_node)
        _draw_arrow(top_right_node)
